...
export function CommandMenu({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  const itemCls =
    "flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 text-[13px] data-[selected=true]:bg-muted [&_svg]:size-4 [&_svg]:text-muted-foreground";
  const groupCls =
    "[&_[cmdk-group-heading]]:px-3 [&_[cmdk-group-heading]]:pb-1 [&_[cmdk-group-heading]]:pt-3 [&_[cmdk-group-heading]]:text-[11px] [&_[cmdk-group-heading]]:font-medium [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-wider [&_[cmdk-group-heading]]:text-muted-foreground";
              ...
              <Command.Group heading="Modules" className={groupCls}>
                {allNavItems.map((i) => (
                  <Command.Item key={i.href} value={`${i.title} ${i.summary}`} onSelect={() => go(i.href)} className={itemCls}>
                    <i.icon />
                    <span>{i.title}</span>
                    <span className="ml-auto truncate text-[11px] text-muted-foreground">{i.summary}</span>
                  </Command.Item>
                ))}
              </Command.Group>
              ...
              <Command.Group heading="Videos" className={groupCls}>
                {videos.map((v) => (
                  <Command.Item key={v.id} value={`video ${v.code} ${v.title}`} onSelect={() => go(`/production/${v.id}`)} className={itemCls}>
                    <Film />
                    <span className="font-mono text-[12px] text-muted-foreground">{v.code}</span>
