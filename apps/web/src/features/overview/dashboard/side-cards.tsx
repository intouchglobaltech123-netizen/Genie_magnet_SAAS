...
export function ReviewCountdownCard() {
  ...
  return (
    <Card className="relative overflow-hidden border-accent/25 bg-grid">
      ...
      <CardContent className="relative flex items-center gap-5 p-5">
        <div className="relative size-20 shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-[22px] font-semibold leading-none tabular">{days}</span>
            <span className="text-[10px] text-muted-foreground">days</span>
          </div>
        </div>
        <div className="min-w-0 flex-1">
          <div className="text-[11px] font-medium uppercase tracking-wider text-accent">Next 45-day strategic review</div>
          <div className="mt-1 text-[15px] font-semibold">Saturday, 10 Oct 2026 · 10:00 AM</div>
          <div className="mt-0.5 text-[12.5px] text-muted-foreground">Goals, client health, margins, founder dependency. 4 of 7 pre-reads ready.</div>
    ...
    <Card>
      ...
      <CardContent className="space-y-2.5">
        {upcoming.map((s) => {
          ...
          return (
            <Link key={s.id} href="/shoots" className="flex items-center gap-3 rounded-xl border border-border p-3 transition hover:border-accent/40 hover:bg-muted/40">
              <div className="flex w-11 shrink-0 flex-col items-center rounded-lg bg-muted py-1">
                ...
                <span className="text-[17px] font-semibold leading-tight tabular">{format(d, "d")}</span>
              </div>
              <div className="min-w-0 flex-1">
...
const dotTone = { accent: "bg-accent", success: "bg-success", warning: "bg-warning", danger: "bg-danger" };
        ...
        <ol className="relative space-y-3.5 before:absolute before:bottom-1 before:left-[3px] before:top-1 before:w-px before:bg-border">
          {activity.slice(0, limit).map((a) => (
            <li key={a.id} className="relative pl-5">
              ...
              <div className="mt-0.5 text-[11px] text-muted-foreground tabular">{format(parseISO(a.at), "d MMM · HH:mm")}</div>
            </li>
          ))}
        </ol>
      </CardContent>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  <span className="truncate text-[12.5px] font-medium">
                    {p.name}
                    {p.type === "freelancer" && <span className="ml-1 text-[10.5px] font-normal text-muted-foreground">freelance</span>}
                  </span>
                  <Tooltip content={over ? "Over capacity — rebalance" : p.status === "on-leave" ? "On leave" : `${pct(p.utilisation)} of available hours`}>
                    <span className={cn("text-[12px] font-semibold tabular", over ? "text-danger" : low ? "text-muted-foreground" : "")}>
                      {pct(p.utilisation)}
                    </span>
                  </Tooltip>
                </div>
                <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-muted">
                  <div
                    className={cn("h-full rounded-full", over ? "bg-danger" : p.utilisation > 0.9 ? "bg-warning" : low ? "bg-chart-5" : "bg-accent")}
