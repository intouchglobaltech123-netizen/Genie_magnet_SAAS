    ...
    <Card>
      <CardHeader>
        ...
        <div className="flex gap-5 text-right">
          <div>
            <div className="text-[11px] text-muted-foreground">Contracted (6 mo)</div>
            <div className="text-[15px] font-semibold tabular">{inrCompact(totalContracted)}</div>
          </div>
          <div>
            <div className="text-[11px] text-muted-foreground">Collected (6 mo)</div>
            <div className="text-[15px] font-semibold tabular">{inrCompact(totalCollected)}</div>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={revenueTrend} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
              ...
              <Tooltip {...tooltipStyle} cursor={{ stroke: "var(--border)" }} formatter={(v) => inr(Number(v))} />
              <Area type="monotone" dataKey="contracted" name="Contracted" stroke="var(--chart-1)" strokeWidth={2} fill="url(#gContracted)" />
              <Area type="monotone" dataKey="collected" name="Collected" stroke="var(--chart-2)" strokeWidth={2} fill="url(#gCollected)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </CardContent>
    ...
    <Card className="flex flex-col">
      ...
      <CardContent className="flex-1 space-y-2">
        {counts.map(({ stage, count }) => (
          <Link key={stage} href="/production" className="group grid grid-cols-[112px_1fr_24px] items-center gap-3 text-[12.5px]">
        ...
        <div className="flex items-center gap-5">
          <div className="relative size-32 shrink-0">
            ...
            <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-[15px] font-semibold tabular">{inrCompact(total)}</span>
            ...
            {clients
              ...
              .map((c) => (
                <div key={c.id} className="flex items-center gap-2">
                  ...
                  <span className="w-12 text-right text-[12px] text-muted-foreground tabular">{inrCompact(c.monthlyValue)}</span>
...
export function HealthPill({ value }: { value: number }) {
  ...
  return <span className={cn("inline-flex w-9 justify-center rounded-md py-0.5 text-[11px] font-semibold tabular", tone)}>{value}</span>;
}
