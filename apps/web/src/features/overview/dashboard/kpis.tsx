    ...
    <Card className="relative overflow-hidden p-5">
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 opacity-60">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={revenueTrend} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="collected" stroke="var(--chart-1)" strokeOpacity={0.35} strokeWidth={1.5} fill="url(#heroFill)" isAnimationActive={false} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      <div className="relative">
        <div className="flex items-start justify-between">
          <div>
            <div className="text-[13px] font-medium text-muted-foreground">Revenue · September 2026</div>
            <div className="mt-2 flex items-baseline gap-2">
              <span className="text-[34px] font-semibold leading-none tracking-tight tabular">{inrCompact(sep.contracted)}</span>
              <span className="text-[13px] text-muted-foreground">contracted / month</span>
            </div>
          </div>
  ...
  return (
    <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
      ...
      <Link href="/crm" className="group">
        <StatCard label="Weighted pipeline" value={inrCompact(weighted)} icon={Target} tone="gold" className="h-full transition group-hover:border-accent/40">
