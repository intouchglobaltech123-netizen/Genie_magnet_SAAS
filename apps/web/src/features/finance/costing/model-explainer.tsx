...
const blocks = [
  ...
  },
  {
    ...
    icon: Building2,
    color: "bg-chart-3/20 text-accent-strong",
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 shadow-card">
      <div className="mb-4 flex flex-wrap items-baseline justify-between gap-2">
        <div>
          <h3 className="text-subheading font-semibold text-text-primary">How true cost is built</h3>
          <p className="mt-0.5 text-body text-muted-foreground">
            Every figure on this page follows one formula. Hover a block to see the rule behind it.
          </p>
        </div>
        <div className="flex items-center gap-1.5 rounded-lg bg-muted px-2.5 py-1 text-body font-medium text-text-secondary">
      ...
      <div className="flex flex-col items-stretch gap-2 lg:flex-row lg:items-center">
        {blocks.map((b, i) => (
          <div key={b.key} className="flex flex-1 items-center gap-2 lg:min-w-0">
            <Tooltip content={b.detail}>
              <div tabIndex={0} className="group min-w-0 flex-1 cursor-help rounded-xl border border-border bg-surface-secondary p-3 transition hover:border-primary/40 hover:bg-primary-soft/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/30">
                <div className="flex items-center gap-2">
                  <span className={cn("inline-flex size-6 items-center justify-center rounded-lg", b.color)}>
        ...
        <div className="flex items-center justify-center rounded-xl bg-primary px-4 py-3 text-center text-primary-foreground lg:w-28">
          <div>
            <div className="text-body opacity-80">per video</div>
