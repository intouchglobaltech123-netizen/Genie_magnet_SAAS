  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-end gap-3">
          <div className="w-[88px] space-y-1.5">
            <div className="text-[11.5px] font-medium text-muted-foreground">Day</div>
            <Select
              value={String(day)}
          ...
          <div className="min-w-[200px] flex-1 space-y-1.5">
            <div className="text-[11.5px] font-medium text-muted-foreground">What did I spend on?</div>
          ...
          <div className="w-[120px] space-y-1.5">
            <div className="text-[11.5px] font-medium text-muted-foreground">Amount</div>
          ...
          <div className="space-y-1.5">
            <div className="text-[11.5px] font-medium text-muted-foreground">Need / Want / Craving</div>
            <Segmented
              value={kind}
        ...
        <div className="mt-3 flex flex-wrap items-end gap-3">
          <div className="w-[170px] space-y-1.5">
            <div className="text-[11.5px] font-medium text-muted-foreground">Leak type</div>
            <Select
              value={leakType}
          ...
          <div className="w-[200px] space-y-1.5">
            <div className="text-[11.5px] font-medium text-muted-foreground">Emotion before spend</div>
            <Select value={emotion} onValueChange={(v) => setEmotion(v as Emotion)} options={EMOTIONS.map((e) => ({ value: e, label: e }))} />
          </div>
          <div className="space-y-1.5">
            <div className="text-[11.5px] font-medium text-muted-foreground">Mood after</div>
          ...
          <div className="ml-auto flex items-end gap-3">
            <div className="text-right">
              <div className="text-[11px] text-muted-foreground">Leakage · 15-yr cost</div>
              <div className={cn("text-[14px] font-semibold tabular", previewLeak > 0 ? "text-danger" : "text-success")}>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-1.5">
          <span className="text-[11.5px] text-muted-foreground">Quick:</span>
          {quickPicks.map((q) => (
            <button key={q} onClick={() => setItem(q)} className="cursor-pointer rounded-full border border-border px-2.5 py-0.5 text-[12px] text-muted-foreground transition hover:border-accent/40 hover:text-foreground">
              {q}
            </button>
          ))}
        </div>
      ...
      <Card className="overflow-hidden">
        <div className="hidden grid-cols-[1fr_96px_92px_120px_170px_48px_72px_96px_96px_36px] gap-3 border-b border-border px-4 py-2.5 text-[11px] font-medium uppercase tracking-wider text-muted-foreground lg:grid">
          <span>Spend</span>
        ...
        {byDay.length === 0 && (
          <div className="px-4 py-16 text-center text-sm text-muted-foreground">No spends here yet. Add your first one above — be 100% honest with yourself.</div>
        )}
        {byDay.map(([d, entries]) => {
              ...
              <AnimatePresence initial={false}>
                {entries.map((e) => (
                  <motion.div
                    ...
                    className="group grid grid-cols-2 items-center gap-x-3 gap-y-1.5 px-4 py-2.5 text-[13px] lg:grid-cols-[1fr_96px_92px_120px_170px_48px_72px_96px_96px_36px]"
                  >
                    <div className="min-w-0">
                      <div className="truncate font-medium">{e.item}</div>
                      {e.notes && <div className="truncate text-[11.5px] text-muted-foreground">{e.notes}</div>}
                    </div>
                    <div className="text-right font-medium tabular">{inr(e.amount)}</div>
                    <div>
                      <KindPill kind={e.kind} />
                    </div>
                    <div className="text-[12px] text-muted-foreground">{e.leakType === "NONE" ? "—" : e.leakType}</div>
                    <div className="truncate text-[12px]">{e.emotion}</div>
                    <div>
                    ...
                    <div>
                      <Tooltip content="Click to change · only 'No' on a Want counts as leakage">
                        <button
                          ...
                          className={cn(
                            "cursor-pointer rounded-md px-1.5 py-0.5 text-[11.5px] font-medium",
      ...
      <CardContent className="flex items-start gap-2 p-0 text-[12px] text-muted-foreground">
